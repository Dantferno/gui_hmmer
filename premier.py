...
def download_pfam(last_modified,ftp,frame):
    ...
    #decompresse le fichier et suprrime l'ancienne librairie
    ...
    #suprrime les anciens indexes et hmm press la nouvelle librairie
...
def troisieme_fenetrefunc():
    ...
    send_check = tk.Button(troisieme_fenetre,text='Ajouter a la base de donnee',command=popup)
    tree.grid()
    send_check.grid()
...
def deuxieme_fenetrefunc(macmd):
    #Deuxieme fenetre
    ...
    #output dynamique dans output_text
    progress_bar = ttk.Progressbar(deuxieme_fenetre, length=650,maximum=17929)
    def popene(output_text,macmd,progress_bar):
        ...
        if p.returncode !=0:
            ...
            suivant = tk.Button(deuxieme_fenetre,text='suivant',command=troisieme_fenetrefunc).grid()
...
def check_input(file,choicecmd,evalue):
    ...
    if check_fasta(file) is False:
        ...
        if choicecmd=='hmmsearch':
            ...
            command = '{0} --domtblout tmp --noali -E {2} ./libraire/Pfam-A.hmm {1} '.format(choicecmd,filename,evalue)
        deuxieme_fenetrefunc(command)
...
def accueil_fenetre():
    ...
    import_resultat = tk.Button(accueil, text='Importer ses propres resultats')
    maj.grid(row=5)
    suivant.grid(row=6)
